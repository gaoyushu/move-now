...
export default class CommentsW extends Component {
    ...
    pv=()=>{
        ...
        if(this.props.location.pathname.indexOf('open')!==-1){
            fetch('http://116.62.14.0:8666/square/comments/1', {
            method: 'POST',
            headers: {
            'Accept': 'application/json',
            'Content-Type': 'text/plain'
            },
            body: JSON.stringify({ did:parseInt( this.props.location.pathname.split('/')[4]),comcontent:`${this.state.vc}`, comstatus:"true"})})
            .then(res =>{ return res.json() })
            .then(res =>{ 
                console.log(res); 
                if(res.data==='ok'){
        ...
        fetch('http://116.62.14.0:8666/square/comments/1', {
            ...
            body: JSON.stringify({ did:brr[5],comcontent:`${this.state.vc}`, comstatus:"true"})})
            .then(res =>{ return res.json() })
            .then(res =>{ 
                console.log(res); 
                if(res.data==='ok'){
