...
export default class SquareBranch extends Component {
    constructor(props){
      ...
      componentDidMount(){
        ...
        var datas=arr[2];
        console.log(datas);
            fetch('http://116.62.14.0:8666/square/'+datas+'/'+localStorage.getItem('token')).then(res =>res.json())
                .then(res =>{
             console.log(res); 
             this.setState({
                 data:res.data
      ...
      setLike=(idx)=>{
          console.log(this.state.data[idx].did)
            fetch('http://116.62.14.0:8666/square/praise/'+localStorage.getItem('token')+'/'+`${this.state.data[idx].did}`,{
                method: 'PUT',
                headers: {
