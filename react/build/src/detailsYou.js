...
export default class DetailsYou extends Component {
    ...
    componentDidMount(){
        ...
        if(this.props.location.pathname.indexOf('open')!==-1){
            let did=this.props.match.params.did;
                fetch('http://116.62.14.0:8666/diary/detail/'+did+'/'+localStorage.getItem('token'))
            ...
            fetch('http://116.62.14.0:8666/diary/detail/'+didss+'/'+localStorage.getItem('token'))
            .then(res =>{ return res.json() })
            .then(res =>{
                console.log(res); 
            if(res.data.dimage===-1){
    ...
    handleclick1=()=>{
        fetch('http://116.62.14.0:8666/square/praise/'+localStorage.getItem('token')+'/'+`${this.state.data.did}`,{
                method: 'PUT',
                headers: {
