...
export default class Diary extends Component {
    constructor(){
        ...
        this.state={
            ...
            month:'',
            year:'',
            privateImg:'http://116.62.14.0:8666/api/image/41',
            publicImg:'http://116.62.14.0:8666/api/image/40'
            // zIndex:1000
    ...
    componentDidMount(){
        ...
        .then(res =>{ 
         ...
         fetch('http://116.62.14.0:8666/diary/list/1').then(res =>{ return res.json() })
         .then(res =>{ 
    ...
    setChange=(idx)=>{
        ...
        fetch('http://116.62.14.0:8666/diary/type/'+this.state.list[idx].did, {
            ...
            body: JSON.stringify({ token:1, diaryid:`${this.state.list[idx].did}`})})
    ...
    render() {
        ...
        <p style={{fontSize:'12px',textAlign:'center',marginTop:'12px'}}>{this.state.metto.author}</p>
                    ...
                    {this.state.list.map((item,idx)=>(
                        <div className='message-diary' style={{marginTop:5,position:'relative'}} onChange={()=>this.setTime()}>
