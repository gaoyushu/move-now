...
export default class Select1 extends Component {
    ...
    componentDidMount(){
        fetch('http://116.62.14.0:8666/changed/choose/4')
        ...
        this.state.checked.map((item,idx)=>{
            ...
            if(item===true){
                this.setState({
                    ...
                    setTimeout(()=>{
                                    this.props.history.push({pathname:'/apply',state:{
                                        did:this.state.id,
                                        shortdes_id: this.props.location.state.shortdes_id,
                                        ok:this.state.ok
                                    }})
                                },1000)
                //     fetch('http://116.62.14.0:8711/changed/choose', {
                //     method: 'POST',
                //     headers: {
                //         'Accept': 'application/json',
                //         'Content-Type': 'application/json'
                //     },
                //     body: JSON.stringify({ token:1,oneid:`${this.props.location.state.shortdes_id}`,diaryid:`${this.state.id}`})})
                //     .then(res =>{ return res.json() })
                //     .then(res=>{ 
