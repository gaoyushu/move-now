...
export default class Sentence extends Component {
   ...
   fun=()=>{
    ...
    fetch('http://116.62.14.0:8666/change/add', {
        ...
        body: JSON.stringify({token:localStorage.getItem('token'),title:`${this.state.dtitle}`, content:`${this.state.dcontent}`,diaryid:`${this.props.location.state.did}`,})})
        .then(res =>{ return res.json() })
        .then(res=>{ 
            console.log(res); 
            if(res.status===0){
    ...
    render() {
        ...
        return (
                ...
                <div className='comment'>
                    ...
                    <input type='text' className='comment-name' placeholder='一句话介绍~' style={{paddingLeft:10,marginTop:60}} value={this.state.dtitle} onChange={this.title}></input>
                    <textarea rows="13" cols="30" className='comment-area' placeholder='补充一下~' style={{padding:10,marginTop:3}} value={this.state.dcontent} onChange={this.content}>补充一下~</textarea>
