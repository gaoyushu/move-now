...
export default class Siders extends Component {
  ...
  render() {
    let arr = [
      { value: "主页", pathname: `/enter/home` },
      { value: "系统管理", pathname: `/enter/system` },
      { value: "用户管理", pathname: `/enter/user` },
      { value: "日记管理", pathname: `/enter/diary` },
      { value: "评论管理", pathname: `/enter/comments` },
      // { value: "日记审核", pathname: `/enter/diaryexamine` },
      { value: "图片管理", pathname: `/enter/picture` },
    ];
    let list = [];
    arr.map((item, key) =>
      list.push(
        <Link to={{ pathname: `${item.pathname}` }} key={key}>
          <li
            key={key}
            onClick={this.setCurrentIndex}
            index={key}
            className={this.state.currentIndex === key ? "active" : ""}
          >
            {item.value}
          </li>
        </Link>
